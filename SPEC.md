...
本ハンズオンは、**AWS案件参画時のアピール用に、最小構成で実際に動くものを作る**方針とします。  
...
- AWS案件で説明しやすいサーバーレス構成を、実装可能な範囲で形にする
- 面談やポートフォリオで、AWSサービス選定理由と構成意図を説明できるようにする
...
- AWS案件への参画を目指す場で、技術アピール材料を求められるケース
- 面談や職務経歴補足で、サーバーレス設計・実装経験を説明したいケース
...
- ステータスとして少なくとも `RECEIVED` `PROCESSING` `COMPLETED` `FAILED` を扱えること
...
flowchart LR
  ...
  APIGW -->|2. Invoke| Lambda["Lambda (Java)"]
  Lambda -->|3. PutItem| DDB["DynamoDB\nrequests table"]
  ...
  Client -->|8. GET /requests/{id}| APIGW
  Client -->|9. GET /requests| APIGW
```
...
- `Lambda (Java)`
  - APIリクエストを受けて、DynamoDB保存とPresigned URL発行を行う
- `DynamoDB`
  - 受付情報と状態を保存する
- `S3`
  - ファイル本体を保存する
...
- 非同期処理が必要になった場合は、S3イベントやEventBridgeを追加する
...
- AWS案件の説明では、サービス間の接続関係をコンソールで理解していること自体が有効なアピールになる
...
- 面談で説明できるよう、構成意図を言語化しやすい設計を採用する
...
- 最小構成に寄せることで、アピールできる範囲は広すぎず狭すぎない線引きが必要

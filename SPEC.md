...
- クライアント向けのアップロード完了は、Presigned URLに対するS3 PUTの成功（HTTP 2xx）で判定する
- `status=COMPLETED` はバックエンドの非同期反映として扱い、同一レスポンスでの同期保証は行わない
- そのため、状態参照が必要な場合のみ `GET /requests/{id}` で確認する
- 本時点では、S3 PUT成功後に完了通知用の追加LambdaレスポンスAPIは実装しない
...
- 利用者画面での「アップロード完了」は、S3 PUT成功時点で表示してよい
- DynamoDBへの `COMPLETED` 反映は非同期追従であり、ユーザー通知の必須条件にはしない
- この分離により、完了通知のためだけの同期連携を追加せず、構成を単純に保つ
...
- 現フェーズの完了条件は、次の2点を満たすこととする
  - S3アップロード後の `status=COMPLETED` 反映を確認できること
  - AWS本番環境でAPI/Lambdaが起動し、主要疎通が確認できること
- クライアント実装は本フェーズに含めない
- クライアント実装は次フェーズとして分離し、別枠で計画・実装・検証する
